...
const enoughAccountBalance = `-- name: EnoughAccountBalance :one
SELECT (balance >= $1) FROM accounts WHERE id = $2
`

type EnoughAccountBalanceParams struct {
	Balance int64 `json:"balance"`
	ID      int64 `json:"id"`
}

func (q *Queries) EnoughAccountBalance(ctx context.Context, arg EnoughAccountBalanceParams) (bool, error) {
	row := q.db.QueryRowContext(ctx, enoughAccountBalance, arg.Balance, arg.ID)
	var column_1 bool
